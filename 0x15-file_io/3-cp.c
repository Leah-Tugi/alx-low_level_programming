...
void close_file(int file_display);
 ...
 * @file_display: The file descriptor to be closed.
 */
void close_file(int file_display)
{
	int i;

	i = close(file_display);

	if (i == -1)
	{
		dprintf(STDERR_FILENO, "Error: Can't close file_display %d\n", file_display);
		exit(100);
	}
}
